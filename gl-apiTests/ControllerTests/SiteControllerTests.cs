...
public class SiteControllerTests
{
    ...
    public async Task When_Called_Expect_ReturnsCorrectSites()
    {
        ...
        sites.Add(new Site("Cleveland"){Id=1,Altitude=2400,SeismicZone=0});
        sites.Add(new Site("Sacramento"){Id=2,Altitude=10,SeismicZone=2});
        sites.Add(new Site("San Andreas"){Id=3,Altitude=223,SeismicZone=4});

        repoMock.Setup(r=>r.GetSites())
            .Returns(sites); //Task.FromResult(sites));
